...
async def scanPorts(ip):
    for port in ports:
        ...
        print(addr)
        status = await connAddr(s, addr)
        printStatus(status)

async def connAddr(s, addr):
    try:
        s.connect(addr)
        return '[+] {} open'.format(addr)
    except BlockingIOError:
        #return None
        return '[-] {} close. BlockingIOError'.format(addr)
    except Exception as e:
        #return None
        return '[-] {} close. error:{}'.format(addr, e)
        ...
        for CIDR_IPS in f:
            ...
            for i, ip in enumerate(ips):
                if i % IP_NUM != IP_NUM_1:
                    ...
                    ip_list.append(ip)
                    tasks = [scanPorts(ip) for ip in ip_list]
